...
set COLOR=%4
set COLOR_ERROR=4
set WDIR=%MDIR%\ToolProjects
set JYTHON_STAND_JAR=%EDIR%\\ToolBuild\javalib\jython.jar
set APPLICATION_TITLE=ToolBuild Project Builder
...
:main
	...
	color %COLOR%
	cls
	echo toolbuild project automation:
	echo commands:        
	echo.                      
...
:createProject
	...
	mkdir %proj_name%
		...
		set polyglot=%proj_name%\polyglot
		
		mkdir %app_source% %view_source% %model_source% %controller_source%
		mkdir %database_source% %data_source% %test_source% %resources% %polyglot%
